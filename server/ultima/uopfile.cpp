...
auto uopfile::hashLittleFor(const std::string &hashstring, std::size_t index) const ->std::uint64_t{
	auto formatted = strutil::format(hashstring,index) ;
...
auto uopfile::nonIndexHash(std::uint64_t hash, std::size_t entry, std::vector<std::uint8_t> &data) ->bool{
	std::cerr << "Hashlookup failed for entry "s << entry << " with a hash of " << strutil::ntos(hash,strutil::radix_t::hex,true,16)<<std::endl;
	return false;
}

//===============================================================
auto uopfile::loadUOP(const std::string &filepath, std::size_t max_hashindex , const std::string &hashformat1,tileinfo *info, const std::string &hashformat2 )->bool{
	std::ifstream input(filepath, std::ios::binary);
	if (!input.is_open()){
		return false ;
	}
	...
	for (auto &entry : entries){
		...
		if ((entry.identifer != 0 ) && (entry.compressed_length != 0)) {
			...
			if (processHash(entry.identifer, current_entry, uopdata)) {
				...
				processEntry(current_entry, index, uopdata,info);
			}
		}
		current_entry++ ;
	}
...
auto uopfile::writeUOP(const std::string &filepath) ->bool {
	constexpr std::int32_t table_size = 100 ;
	constexpr std::int64_t first_table = 0x200 ;
	constexpr std::uint32_t timestamp = 0xFD23EC43 ;
	constexpr char pad = 0 ;
	constexpr std::int32_t zero = 0 ;
	constexpr std::int64_t bigzero = 0 ;
